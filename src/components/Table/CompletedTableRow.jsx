...
const CompletedTableRow = ({ ticket, index }) => {
  const [showDropdown, setShowDropdown] = useState(false);
  const [currentAgent, setCurrentAgent] = useState(ticket.assignedTo);
  const openUpwards = index >= 5;

  return (
    <div className="grid grid-cols-[1.2fr_2fr_0.8fr_1fr_1.5fr_1.2fr_1.2fr_1fr] gap-4 px-6 border-b border-gray-100 items-center text-[13px] bg-white hover:bg-[#ebf5ff] transition-colors duration-200 cursor-pointer h-[36px] overflow-visible whitespace-nowrap">
      ...
      <div className="text-gray-600 text-xs text-left leading-none truncate shrink-0">{ticket.resolvedOn}</div>
      <div className="text-gray-800 text-[13px] text-left leading-none truncate shrink-0 whitespace-nowrap">{ticket.trt}</div>
    </div>
  );
};
